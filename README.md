...
3. **CLI inside WSL + [Remote Control](https://code.claude.com/docs/en/remote-control.md)**: makes a WSL session controllable from claude.ai/code in a browser, the mobile app, and — working in practice, though officially still an [open feature request](https://github.com/anthropics/claude-code/issues/29006) — the Windows desktop app. Execution is fully Linux-side with desktop UI, which makes this the most integrated working setup today. **Prefer the server-mode subcommand for anything long-running:** `claude remote-control` (no dashes) has documented reattach flags — `--continue` resumes the most recent Remote Control session from the directory, `--session-id <id>` a specific one (both v2.1.200+) — which is exactly what a crash-restart wrapper needs. The interactive flag form (`claude --remote-control`) hides its sessions from the normal resume picker; `--remote-control --resume <id>` works but is undocumented and [closed as not-planned](https://github.com/anthropics/claude-code/issues/56294). Remaining caveats: the process must stay alive (tmux, or a container with a restart policy running `claude remote-control --continue`), both forms still need a TTY ([headless mode is an open request](https://github.com/anthropics/claude-code/issues/30447)), and a session that loses network for >10 minutes exits.
4. **Desktop app → SSH → sshd inside WSL** (plausible, unverified for WSL): the desktop app's SSH environment pointed at a loopback sshd running in the distro would give desktop UI with true Linux-side execution — the closest analog of VS Code Remote-WSL. Untested; the docs don't address WSL as an SSH target specifically. Setup gotcha: WSL distros frequently run without systemd as init (check for "System has not been booted with systemd" from `systemctl`), in which case an installed, "enabled" sshd never actually starts — enable systemd via `[boot] systemd=true` in `/etc/wsl.conf` and `wsl --shutdown`, or start sshd manually per boot.
